    ...
    <div className="py-8 sm:py-12 bg-slate-50 min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12 mb-20">
        ...
        <section className="space-y-6">
          ...
          {/* Overview Card */}
          <div className="p-6 sm:p-8 bg-white border-l-4 border-blue-600 rounded-3xl shadow-xs border-y border-r border-slate-200 space-y-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <h2 className="font-serif font-bold text-xl sm:text-2xl text-slate-900 mt-0.5">
          ...
          <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-xs space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {GOVERNING_BODY_FUNCTIONS.map((func, idx) => (
                <div key={idx} className="p-5 bg-slate-50 rounded-2xl border border-slate-200 shadow-2xs space-y-2 flex flex-col justify-between hover:border-blue-600 transition-colors">
                  <div className="flex items-start gap-3">
                    <div className="w-7 h-7 bg-blue-600 text-white rounded-lg flex items-center justify-center font-mono font-bold text-xs shrink-0 shadow-2xs">
                      {idx + 1}
                    </div>
        ...
        <section className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-sm space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
            ...
            <span className="text-xs font-mono font-bold text-amber-900 bg-amber-100 px-3 py-1.5 rounded-xl border border-amber-300">
              13 Apex Members
            </span>
          </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm">
              <thead className="bg-slate-900 text-white font-serif font-bold uppercase text-[11px]">
                <tr>
                  ...
                  <th className="py-3.5 px-4 text-slate-200">Designation</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 text-slate-700 font-medium">
                {GOVERNING_BODY_MEMBERS.map((member) => {
                  let badgeStyle = "bg-slate-100 text-slate-800 border-slate-200";
                  if (member.portfolio.includes('Management')) badgeStyle = "bg-amber-100 text-amber-900 border border-amber-300 font-bold";
                  else if (member.portfolio.includes('Member Secretary')) badgeStyle = "bg-blue-600 text-white font-bold shadow-2xs";
                  else if (member.portfolio.includes('Nominee')) badgeStyle = "bg-blue-50 text-blue-900 border border-blue-200 font-bold";
                  else if (member.portfolio.includes('Industry Expert')) badgeStyle = "bg-purple-100 text-purple-900 border border-purple-200 font-bold";
                  else if (member.portfolio.includes('Member')) badgeStyle = "bg-slate-100 text-slate-800 border border-slate-200 font-medium";

                  return (
                    <tr key={member.sno} className="hover:bg-blue-50/30 transition-colors">
                      <td className="py-4 px-4 font-mono font-bold text-center text-blue-700">{member.sno}</td>
                      <td className="py-4 px-4 font-bold text-slate-900 whitespace-nowrap">{member.name}</td>
